...
public class UrlPictureUpload extends PictureUploadTemplate {
    ...
    protected String getOriginFilename(Object inputSource) {
        String fileUrl = (String) inputSource;
            // 1. 发送 HEAD 请求获取响应头
            HttpResponse headResponse = HttpUtil.createRequest(Method.HEAD, fileUrl).execute();
            String contentType = headResponse.header("Content-Type");
            // 2. 根据 Content-Type 映射后缀（示例映射）
            String suffix = "";
            if (contentType != null) {
                if (contentType.contains("image/jpeg")) suffix = ".jpg";
                else if (contentType.contains("image/png")) suffix = ".png";
                else if (contentType.contains("image/webp")) suffix = ".webp";
                else suffix = ".jpg"; // jpg 格式默认兜底
            }
            // 3. 在图片文件名后面添加格式后缀，并返回
